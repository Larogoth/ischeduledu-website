...
const Index = () => {
  ...
  const VersionToggle = () => (
    <div className="fixed top-20 right-4 z-50 bg-white rounded-lg shadow-lg p-3 border border-gray-200">
      <div className="flex items-center gap-3">
        <span className={`text-sm font-medium ${!showVersionB ? 'text-[#0FA0CE]' : 'text-gray-500'}`}>
          Version A
        </span>
        <Button
          variant="ghost"
          size="sm"
          onClick={() => setShowVersionB(!showVersionB)}
          className="p-1 h-auto"
        >
          {showVersionB ? (
            <ToggleRight className="w-6 h-6 text-[#0FA0CE]" />
          ) : (
